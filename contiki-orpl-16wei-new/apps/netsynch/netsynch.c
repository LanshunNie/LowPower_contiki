...
#else
#define REQUEST_PERIOD (3*60* CLOCK_SECOND) //15*CLOCK_SECOND
#endif

#define NETSYNC_SEQ_DIFF_MAX   3   //4
...
static clock_time_t
random_delay(){
  return  (CLOCK_SECOND/2 + random_rand()%((FWD_DELAY()*uip_ds6_nbr_num()*3/2)+8*CLOCK_SECOND));
}
#endif
/*---------------------------------------------------------------------------*/
int
...
//          printf("levelllllll:%d\n", authority_level);
        ...
        //  }
          if(netsync_init_flag){
            if(get_idle_time() >2* 60 +10){
             ...
             ctimer_set(&send_netsync_ct,10*CLOCK_SECOND,netsynch_send,NULL);
          //  netsynch_send(NULL);
   ...
   if(netsynch_is_synched()){
    ...
    if(msg.authority_level< AUTHORITY_LEVEL_UNSYNCHED){
      ...
      if(msg.authority_level <= netsynch_authority_level() &&netsyn_offset <= 480 && netsyn_offset >= -480){
        netsync_flag = 1;
      }
